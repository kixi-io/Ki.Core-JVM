...
class Ki {
    ...
    // TODO
    class QuantityDef(nullable:Boolean, val unit:Unit, val numType:Ki.Type) :
        TypeDef(Ki.Type.Quantity, nullable) {

        val nullChar = if (nullable) "?" else ""
        val numTypeSuffix = when(numType) {
            Ki.Type.Decimal, Ki.Type.Int -> ""
            Ki.Type.Double -> ":d"
            Ki.Type.Long -> ":L"
            Ki.Type.Float -> ":f"
    ...
    class RangeDef(nullable:Boolean, val valueDef: TypeDef) : TypeDef(Ki.Type.Range, nullable) {
        val nullChar = if (nullable) "?" else ""
        override fun toString() = "$type<$valueDef>$nullChar"
    }

    class ListDef(nullable:Boolean, val valueDef: TypeDef) : TypeDef(Ki.Type.List, nullable) {
        val nullChar = if (nullable) "?" else ""
        override fun toString() = "$type<$valueDef>$nullChar"
    }

    class MapDef(nullable:Boolean, val keyTypeDef: TypeDef, val valueTypeDef: TypeDef) :
        TypeDef(Ki.Type.Map, nullable) {
        val nullChar = if (nullable) "?" else ""
